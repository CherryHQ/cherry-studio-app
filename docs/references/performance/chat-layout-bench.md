...
`slide-in-flight` 是唯一不看滚动位移的判据，因为它盯的那段运动本来就不在滚动里：发送后的
可见位移由入场行自身的 transform 提供，钉顶滚动是瞬时的。少了它，`send-anchor` 的 `scroll`
轨迹会平得像什么都没发生，而 harness 存在的理由正是量这一段。

它只验「装填了、开火了、起飞距离非零」——**曲线形状验不了**，因为轨迹活在 UI 线程的
shared value 里，逐帧回抛给 JS 会把它本身也拖慢。形状只能靠录像取证，做过一次：
...
有一段 283ms 的空档，按帧号等间距去读会把它算成一次「一帧跳 250px」的硬跳。
...
| `viewport-blank` 峰值 | 0 / 0 / 2 | 0 / 0 / 0 | 0 / 0 / 0 |
| `slide-in-flight` 起飞距离 | 616 / 616 / 616 | 616 / 616 / 616 | 616 / 616 / 616 |
| `slide-in-flight` 装填→开火 | 0 / 0 / 0 | 106 / 109 / 112 | 102 / 107 / 105 |

结论：`follow-up-turn` 的 `offset-reversal` 在修键盘缺陷前是逐像素可复现的 **310 / 310 / 310**
——确定性缺陷长这样，噪声不会三轮同值。修掉之后它是 40/24/22 的真噪声，而入场动画改由行的
transform 承担、钉顶滚动改成瞬时之后，这一档**整栏归零**：没有动画滚动，也就没有收尾回弹。
`offsetReversalPx=100` 的余量从此是满的。

`slide-in-flight` 的起飞距离三场景三轮全部 616px，说明它是纯布局推导出来的量（视口 − 底部
内边距 − 钉顶落点），与话题内容无关。`send-anchor` 的「装填→开火」恒为 0ms 是因为新话题里
锚点当帧就位；另两个场景的 ~105ms 是 ready-gate 的静默窗口，属设计内。
...
- **`estimate-collapse` 在前两个场景恒为 252px**，不是漏修的跳动。它发生在列表挂载后的第一次
  ...
  是 56px 和 48px，于是修正 244/252px。此刻两行合计 104px，远小于 874px 的视口——没有可滚动
  距离，修正不产生任何位移。`follow-up-turn` 的 5px 是因为它进的是已有话题，`assistant-empty`
  的均值早已存在（`est=48 → measured=43`）。这一条是估值冷启动，不是缺陷；判据阈值 400px。
- **`viewport-blank` 的旧读数（38→59%）全部是预留空白本身**，判据在量设计而不是缺陷，余量
  一度只剩 1.4 个百分点。已改成先扣 `endSpace` 再判（见上文两条），指标的含义随之变了：旧表
  里这一行的数字与新版不可比，别拿来做趋势。
