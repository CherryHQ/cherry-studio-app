...
| `offset-reversal` 峰值 | 0 / 0 / 0 | 0 / 0 / 0 | 40 / 24 / 22 |
| `gesture-conflict` 窗口 | 0 / 0 / 0 | 2 / 2 / 2 | 0 / 0 / 0 |
| `viewport-blank` 峰值 | 0 / 6 / 0 | 0 / 0 / 0 | 45 / 59 / 58 |

结论：`follow-up-turn` 那一栏在修键盘缺陷前是逐像素可复现的 **310 / 310 / 310**——确定性缺陷
长这样，噪声不会三轮同值。它现在的 40/24/22 才是真噪声；`offsetReversalPx=100` 的阈值余量
不宽，改判据前先重跑三轮看这一档。前两个场景的 0 是修掉 ready gate 越界之后的结果，在那之前
是 24-64px 的近零往返。
