...
| `offset-reversal` 峰值 | 0 / 0 / 0 | 0 / 0 / 0 | 40 / 22 / 22 |
| `gesture-conflict` 窗口 | 0 / 0 / 0 | 2 / 2 / 2 | 0 / 0 / 0 |
| `viewport-blank` 峰值 | 0 / 0 / 0 | 0 / 0 / 0 | 5 / 3 / 3 |
| `slide-in-flight` 总行程 | 616 / 616 / 616 | 616 / 616 / 616 | 616 / 616 / 616 |
| `slide-in-flight` 滚动分担 | 0 / 0 / 0 | 0 / 0 / 0 | 306 / 306 / 306 |
| `slide-in-flight` 装填→开火 | 0 / 0 / 0 | 108 / 111 / 112 | 106 / 107 / 108 |

结论：`follow-up-turn` 的 `offset-reversal` 在修键盘缺陷前是逐像素可复现的 **310 / 310 / 310**
——确定性缺陷长这样，噪声不会三轮同值。它现在的 40/22/22 是原生滚动动画的收尾过冲（实测
+29 出去、−15 回来），`offsetReversalPx=100` 的余量不宽，改判据前先重跑三轮看这一档。

**这一格曾经短暂地变成 0/0/0，那不是改进**：当时钉顶滚动被改成瞬时，没有动画自然没有收尾
回弹——代价是整屏旧内容一帧切走。用 15px 的落位过冲换 306px 的硬切，这笔账反了；判据看不见
后者（见上文第二个盲区）。
...
`send-anchor` 的「装填→开火」恒为 0ms 是因为新话题里锚点当帧就位；另两个场景的 ~107ms 是
ready-gate 的静默窗口，属设计内。
