...
- **`follow-up-turn` 的 `viewport-blank` 从 38/40/40 涨到 45/59/58**，也不是回归。峰值全部落在
  `anchoring` 相位且数值 ≈ `endSpace/viewport`，量的是**预留空白本身**。修掉 gate 越界后钉顶
  到位更快（场景总时长 ~10s → ~8s），采样于是落在预留空白还没被回复填掉的更早时刻。三个
  场景的稳态仍全为 0%。但阈值是 60%，余量已从 20 个百分点缩到 1.4 个——下次这一格再涨，
  先确认是不是稳态也跟着涨了。
