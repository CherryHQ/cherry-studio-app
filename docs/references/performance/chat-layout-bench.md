...
- **`estimate-collapse` 在前两个场景恒为 252px**，不是漏修的跳动。它发生在列表挂载后的第一次
  ...
  距离，修正不产生任何位移。`follow-up-turn` 的 4px 是因为它进的是已有话题，`assistant-empty`
  的均值早已存在（`est=48 → measured=44`）。这一条是估值冷启动，不是缺陷；判据阈值 400px。
