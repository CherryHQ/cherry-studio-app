...
目标语言、界面语言和提示词模板。版本 1 固定为 Chat Completions；构建隔离由容器和凭据组负责。
...
1. `modelConfigurationChanges` 在模型、供应商、凭据和语言偏好写入前通知配置所有者。
...
“模型设置 → 翻译模型”保留模型选择；目标语言在翻译界面选择，缺少模型或连接不兼容等问题
...
Android 未配置翻译模型时还显示 `null · null`。这些问题尚未修复，不能视为验收通过。
