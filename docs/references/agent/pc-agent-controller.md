...
Status: retained prototype, blocked pending migration to the shared Noise/JSON-RPC protocol.
The stacked pairing/configuration foundation is integrated, but Agent connections return
`agent-version` before opening the old transport. Re-pairing cannot enable this prototype.
See the [remote-access plan](../remote-access/README.md) for the replacement wire contract.
The application-facing Agent Protocol remains version 2; the local version 1 Host/Runtime
contract is unchanged. The flows below describe the retained prototype, not device acceptance.
