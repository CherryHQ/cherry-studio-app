...
the whole turn's ten-minute execution deadline. Tools marked `interaction: 'user-input'` pause
that deadline while awaiting a human response and restore its remaining budget afterward. Cancellation and timeout abort the model, approval waiters, and
