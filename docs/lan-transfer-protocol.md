...
| `file_chunk` | Client → Server | 二进制帧 | 文件数据块（无 Base64） |
...
| `file_chunk` | Client → Server | 文件数据块 |
...
#### 5.2.4 `file_chunk_ack` - 数据块确认

**方向：** Server → Client

```typescript
type LanTransferFileChunkAckMessage = {
  type: 'file_chunk_ack'
  transferId: string           // 传输 ID
  chunkIndex: number           // 确认的块索引
  received: boolean            // 是否成功接收
  error?: string               // 错误信息（received=false 时）
}
```
...
// 文件传输（与二进制帧一致）
export const LAN_TRANSFER_CHUNK_SIZE = 64 * 1024             // 64KB
...
// 常量
export const LAN_TRANSFER_TCP_PORT = 53317
export const LAN_TRANSFER_CHUNK_SIZE = 64 * 1024
