...
- **稳态冷启动**：seeder 命中 `app_state` 里的 journal 版本直接 skip，只剩开库 + 迁移检查 + 读两次表（偏好、seed journal）。
...
- 若首启开屏超出可接受阈值，再评估已在第二部分**明确不做**的优化项是否需要重启：`PresetProviderSeeder` 全量目录写入延后到 post-ready、FTS `runCustomMigrations` 版本 guard（见 `docs/adr/0009` / 计划「明确不做」清单）。阈值与优化取舍待本 TODO 有数据后再定。
