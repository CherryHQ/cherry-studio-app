...
describe('outbound tool-output projection', () => {
  ...
  it.each([
    ...
    const fromPart = (
      projectMessagePartForRenderer(partWith(output), TOPIC_ID, MESSAGE_ID) as unknown as {
        output: unknown;
      }
    ).output;
    const fromChunk = (
      projectStreamChunkForRenderer(chunkWith(output), TOPIC_ID, MESSAGE_ID) as unknown as {
        output: unknown;
      }
    ).output;
