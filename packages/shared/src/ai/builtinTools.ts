...
export const kbListInputSchema = z.object({
  query: z
    ...
    .optional()
    .describe(
      'List mode only: case-insensitive substring filter against base name and sample sources.',
    ),
  groupId: z
    .string()
    .trim()
    ...
    .optional()
    .describe(
      'List mode only: restrict the result to a single knowledge base group. Omit to span all groups.',
    ),
  baseId: z
    .string()
    .trim()
  ...
  maxDepth: z
    ...
    .optional()
    .describe(
      'Outline mode only (requires `baseId`): limit the tree to this many folder levels (0 = top level).',
    ),
});
...
export const kbReadInputSchema = z.object({
  ...
  conceptId: z
    ...
    .min(1)
    .describe(
      'Concept ID of the document to read — the `conceptId` field of a kb_search hit (its relative path).',
    ),
  charStart: z
    .number()
    .int()
    ...
    .optional()
    .describe(
      'Read mode only: 0-based start offset of the slice to read. Omit to start at the beginning.',
    ),
  charEnd: z
    .number()
    .int()
  ...
  ignoreCase: z
    .boolean()
    .optional()
    .describe('Grep mode only: case-insensitive matching. Defaults to true.'),
  maxMatches: z
    .number()
    .int()
    .positive()
...
export const kbReadStrictInputSchema = z.object({
  ...
  charStart: z
    ...
    .nonnegative()
    .describe(
      'Read mode only: 0-based start offset of the slice to read. Pass 0 to start at the beginning.',
    ),
  charEnd: z
    .number()
    .int()
  ...
  ignoreCase: z
    .boolean()
    .describe(
      'Grep mode only: case-insensitive matching. Pass true for the default; ignored in read mode.',
    ),
  maxMatches: z
    .number()
    .int()
...
export const kbManageInputSchema = z.object({
  baseId: z
    ...
    .trim()
    .min(1)
    .describe('ID of the knowledge base to modify — a base id from kb_list.'),
  action: z
  ...
  path: z
    ...
    .optional()
    .describe(
      'For action="add", type="file": absolute local filesystem path of the file to import.',
    ),
  url: z
    .string()
    .trim()
    .min(1)
    .optional()
    .describe('For action="add", type="url": the URL to fetch and index.'),
  content: z
    .string()
    .min(1)
    .optional()
  ...
  title: z
    ...
    .optional()
    .describe(
      'For action="add", type="note": optional display title (defaults to the note\'s first line).',
    ),
  conceptIds: z
    .array(z.string().trim().min(1))
    .optional()
...
export const kbManageStrictInputSchema = z.object({
  ...
  url: z
    ...
    .trim()
    .describe(
      'For action="add", type="url": the URL to fetch and index. Pass an empty string otherwise.',
    ),
  content: z
    .string()
    .describe(
      'For action="add", type="note": the plain-text note content to index. Pass an empty string otherwise.',
    ),
  title: z
    .string()
    .trim()
