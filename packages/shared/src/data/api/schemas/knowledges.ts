...
// for those items.
export const UpdateKnowledgeBaseSchema = KnowledgeBaseEntitySchema.pick(
  KNOWLEDGE_BASE_MUTABLE_FIELDS,
)
  .partial()
  .extend({
  ...
  .superRefine((value, ctx) => {
    ...
    if (
      embeddingModelIdProvided &&
      (value.embeddingModelId === null) !== (value.dimensions === null)
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['dimensions'],
