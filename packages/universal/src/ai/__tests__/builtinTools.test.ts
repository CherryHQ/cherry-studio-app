...
describe('builtin tool contracts', () => {
  ...
  it('keeps kb_list strict-path fields in `required` so strict providers accept the schema', () => {
    ...
    expect(json.required).toEqual(
      expect.arrayContaining(['query', 'groupId', 'baseId', 'maxDepth']),
    );
    expect(
      kbListStrictInputSchema.safeParse({ query: '', groupId: '', baseId: '', maxDepth: -1 })
        .success,
    ).toBe(true);
  ...
  it('lets the MCP kb_manage path omit unused fields', () => {
    ...
    expect(
      kbManageInputSchema.safeParse({ baseId: 'kb-1', action: 'add', type: 'note', content: 'hi' })
        .success,
    ).toBe(true);
  });
  ...
  it('lets the MCP kb_read path omit mode-specific fields', () => {
    expect(
      kbReadInputSchema.safeParse({ baseId: 'kb-1', conceptId: 'docs/intro.md' }).success,
    ).toBe(true);
  });
