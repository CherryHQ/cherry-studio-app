...
const OrEntry = z
  .object({
    ...
    architecture: z
      ...
      .optional(),
    // `/api/v1/models` lists it as a string array (feature flags); `/images/models` lists it as a
    // record of typed parameter descriptors. Accept both so one schema parses either endpoint.
...
export function parseOrEntry(raw: unknown): CherryMeta | null {
  ...
  return dropUndef({
    // Only the `/images/models` catalog (object-shaped supported_parameters) carries a curated name.
