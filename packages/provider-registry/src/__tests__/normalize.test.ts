...
describe('normalizeModelId — spelling variants collapse to one canonical', () => {
  it('folds a bedrock cross-vendor arn to the bare canonical id', () => {
    expect(normalizeModelId('us.anthropic.claude-sonnet-4-5-v1:0')).toBe(
      normalizeModelId('claude-sonnet-4-5'),
    );
    expect(normalizeModelId('anthropic.claude-3-5-haiku-20241022:0')).toBe(
      normalizeModelId('claude-3-5-haiku'),
    );
  });
