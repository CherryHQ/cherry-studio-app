...
describe('provider reasoning contracts', () => {
  ...
  it('keeps DeepSeek V4 Pro low efforts mapped to high', () => {
    const wire = override('deepseek', 'deepseek-v4-pro').reasoningContracts?.[
  ...
  it('keeps NVIDIA unknown models fail-closed and declares audited controls per exact model', () => {
    ...
    expect(nvidia.endpointConfigs?.['openai-chat-completions']?.reasoningFormat?.wire).toEqual({
      disabled: true,
    });

    expect(
      override('nvidia', 'qwen3-5-122b-a10b').reasoningContracts?.['openai-chat-completions']?.wire
        ?.auto,
    ).toMatchObject({
      operations: [
        {
    ...
    expect(
      override('nvidia', 'kimi-k2-6').reasoningContracts?.['openai-chat-completions']?.wire?.auto,
    ).toMatchObject({
      operations: [
    ...
    expect(
      override('nvidia', 'deepseek-v4-pro').reasoningContracts?.['openai-chat-completions']?.wire
        ?.effort?.operations,
    ).toEqual([{ target: 'reasoning_effort', value: { source: 'effort' } }]);
    expect(
      override('nvidia', 'deepseek-v4-pro').reasoningContracts?.['openai-chat-completions']?.support
        ?.controls,
  ...
  it('uses each audited NVIDIA model endpoint vocabulary instead of one Nemotron family wire', () => {
    expect(
      override('nvidia', 'minimax-m3').reasoningContracts?.['openai-chat-completions']?.wire?.auto
        ?.operations,
    ).toEqual([
      {
    ...
    expect(
      override('nvidia', 'mistral-small-4-119b').reasoningContracts?.['openai-chat-completions']
        ?.support?.controls,
    ).toEqual([{ kind: 'effort', values: ['none', 'high'], default: 'high' }]);
    expect(
      override('nvidia', 'nemotron-3-super-120b-a12b').reasoningContracts?.[
        'openai-chat-completions'
      ]?.support?.controls,
    ).toEqual([{ kind: 'effort', values: ['none', 'low', 'high'], default: 'high' }]);
    expect(
      override('nvidia', 'nemotron-3-ultra-550b-a55b').reasoningContracts?.[
        'openai-chat-completions'
      ]?.support?.controls,
    ).toEqual([{ kind: 'effort', values: ['none', 'medium', 'high'], default: 'high' }]);
    expect(
      override('nvidia', 'nemotron-3-nano-omni-30b-a3b').reasoningContracts?.[
        'openai-chat-completions'
      ]?.wire?.effort?.operations,
    ).toEqual([{ target: 'reasoning_budget', value: { source: 'budget' } }]);
    expect(
      override('nvidia', 'seed-oss-36b-instruct').reasoningContracts?.['openai-chat-completions']
        ?.wire?.off?.operations,
  ...
  it.each([
    ...
    expect(
      provider('nvidia').overrides?.some(
  ...
  it('nests Poe custom reasoning parameters under extra_body', () => {
    expect(
      override('poe', 'gpt-5-4').reasoningContracts?.['openai-chat-completions']?.wire?.effort
        ?.operations,
    ).toEqual([{ target: 'extra_body.reasoning_effort', value: { source: 'effort' } }]);
    expect(
      override('poe', 'claude-sonnet-4-6').reasoningContracts?.['openai-chat-completions']?.wire
        ?.effort?.operations,
