...
describe('lookupRegistryModel', () => {
  ...
  it('override also uses normalized fallback', () => {
    const result = lookupRegistryModel(
      [],
      [makeOverride('openrouter', 'gpt-4o')],
      'openrouter',
      'gpt-4o:free',
    );
    expect(result.registryOverride!.modelId).toBe('gpt-4o');
...
describe('inferAdapterFamily', () => {
  it('catalog adapterFamily wins over endpoint default', () => {
    expect(
      inferAdapterFamily(ENDPOINT_TYPE.ANTHROPIC_MESSAGES, { adapterFamily: 'aihubmix' }),
    ).toBe('aihubmix');
  });
  ...
  it('accepts both RegistryEndpointConfig and RuntimeEndpointConfig shapes', () => {
    ...
    expect(
      inferAdapterFamily(ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS, { adapterFamily: 'groq' }),
    ).toBe('groq');
  });
});
...
describe('endpointImpliedCapability', () => {
  it('maps capability-exclusive endpoints to their capability', () => {
    ...
    expect(endpointImpliedCapability(ENDPOINT_TYPE.OPENAI_IMAGE_GENERATION)).toBe(
