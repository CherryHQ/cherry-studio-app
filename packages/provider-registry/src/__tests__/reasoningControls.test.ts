...
describe('deriveLegacyReasoningFields', () => {
  it('derives the vocabulary from an effort control', () => {
    expect(
      deriveLegacyReasoningFields([{ kind: 'effort', values: ['low', 'medium', 'high', 'max'] }]),
    ).toEqual({
  ...
  it('derives token limits from a budget control', () => {
    expect(
      deriveLegacyReasoningFields([{ kind: 'budget', min: 1024, max: 64_000, default: 8192 }]),
    ).toEqual({
  ...
  it("a toggle next to an effort control appends 'none' (never index 0 — the legacy coercion fallback)", () => {
    expect(
      deriveLegacyReasoningFields([
        { kind: 'effort', values: ['low', 'high'], default: 'low' },
        { kind: 'toggle' },
      ]),
    ).toEqual({
...
describe('inferReasoningControls (ingest-time heuristics)', () => {
  it.each([
    ...
    [
      'claude-sonnet-5',
      [{ kind: 'effort', values: ['low', 'medium', 'high', 'max'] }, { kind: 'toggle' }],
    ],
    [
      'claude-opus-4-8',
      [{ kind: 'effort', values: ['low', 'medium', 'high', 'max'] }, { kind: 'toggle' }],
...
describe('ReasoningSupportSchema control invariants', () => {
  ...
  it('rejects an effort default outside its values', () => {
    const r = ReasoningSupportSchema.safeParse({
      controls: [{ kind: 'effort', values: ['low'], default: 'high' }],
    });
    expect(r.success).toBe(false);
  });
  ...
  it('rejects a budget default outside [min, max]', () => {
    const r = ReasoningSupportSchema.safeParse({
      controls: [{ kind: 'budget', min: 100, max: 200, default: 300 }],
    });
    expect(r.success).toBe(false);
  });
});
...
describe('catalog invariant: controls ↔ derived legacy fields', () => {
  ...
  const { models } = readCatalogJson<{
    models: Array<{
      id: string;
      reasoning?: { controls?: ReasoningControl[] } & Record<string, unknown>;
    }>;
  }>('models.json');
    ...
    for (const m of models) {
      ...
      for (const [key, value] of Object.entries(derived)) {
        ...
        if (canonical(actual ?? null) !== canonical(value ?? null)) {
          offenders.push(
            `${m.id}: ${key} = ${JSON.stringify(actual)} ≠ derived ${JSON.stringify(value)}`,
          );
        }
      }
    }
