...
describe('RegistryLoader override index — exact apiModelId vs normalized collision', () => {
  ...
  it('exact apiModelId resolves to its own row, not a normalized same-family sibling', () => {
    ...
    expect(loader.findOverride('aws-bedrock', 'google.gemma-3-27b-it')?.modelId).toBe(
      'gemma-3-27b-it',
    );
    expect(loader.findOverride('aws-bedrock', 'meta.llama3-1-8b-instruct-v1:0')?.modelId).toBe(
