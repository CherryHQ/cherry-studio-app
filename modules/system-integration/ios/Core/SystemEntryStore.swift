...
enum SystemEntryStore {
  ...
  static func stageShare(text: String, attachments: [SharedAttachment]) throws {
    ...
    do {
      ...
      for attachment in attachments {
        ...
        total += size
        guard size <= 25 * 1024 * 1024, total <= 50 * 1024 * 1024 else { throw TranslationFailure(code: "shareFailed") }
  ...
  static func complete(_ id: String) throws {
    lock.lock(); defer { lock.unlock() }
    guard UUID(uuidString: id) != nil else { throw TranslationFailure(code: "failed") }
    claimed.remove(id)
...
public enum CherrySystemIntents {
  ...
  public static func ask(_ text: String, agentId: String?) async throws -> String {
    guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty, text.utf16.count <= 131_072 else {
      throw TranslationFailure(code: "invalidInput")
    }
    ...
    while Date() < deadline {
      ...
      if let result = SystemEntryStore.takeReply(id) {
        guard result["status"] as? String == "succeeded", let response = result["text"] as? String else {
          throw TranslationFailure(code: "failed")
        }
        return response
      }
  ...
  public static func translate(_ text: String, targetLanguage: String?) async throws -> String {
    let language = try targetLanguage ?? TranslationConfigurationStore.read().targetLanguage
    return try await TemporaryTranslation.translate(text, targetLanguage: language)
  }
