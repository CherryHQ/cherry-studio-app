...
import type { BackgroundReplyLifecycle, BackgroundReplyTurn } from '../../backgroundReply';
...
describe('ChatSessionImpl', () => {
  ...
  test('drives the background reply lifecycle from the assembled assistant stream', async () => {
    ...
    const backgroundReply = createBackgroundReplyLifecycle(turn);
  ...
  test('passes an empty assistant name to the lifecycle fallback', async () => {
    const services = createServices();
  ...
  test('persists an error status when streaming fails after reservation', async () => {
    ...
    const turn = createBackgroundReplyTurn();
    const runtime = createRuntime({
      backgroundReply: createBackgroundReplyLifecycle(turn),
      services,
    });
    mockReadUIMessageStream.mockReturnValue(failingAsyncIterable(new Error('stream failed')));
    ...
    expect(runtime.getTopicSnapshot('topic-1').status).toBe('idle');
    expect(turn.finish).toHaveBeenCalledWith('failed');
  });
  ...
  test('keeps concurrent topic background turns independent', async () => {
    const services = createServices();
    const turns = new Map<string, BackgroundReplyTurn>();
    const backgroundReply: BackgroundReplyLifecycle = {
      dispose: jest.fn(),
      startTurn: jest.fn(({ topicId }) => {
        const turn = createBackgroundReplyTurn();
        turns.set(topicId, turn);
        return turn;
      }),
    };
    services.topic.getById = jest.fn(async (topicId: string) =>
      createTopic({ id: topicId, name: topicId }),
    );
    services.message.createUserMessageWithPlaceholders = jest.fn(async ({ topicId }) => ({
      placeholders: [{ ...createMessage(`assistant-${topicId}`, 'assistant'), topicId }],
      userMessage: { ...createMessage(`user-${topicId}`, 'user'), topicId },
    }));
    services.message.getPathToNode = jest.fn(async (messageId: string) => [
      { ...createMessage(messageId, 'user'), topicId: messageId.replace('user-', '') },
    ]);
    mockReadUIMessageStream.mockImplementation(({ message }: { message: CherryUIMessage }) =>
      asyncIterable([createUiMessage(message.id, `reply for ${message.id}`)]),
    );
    const runtime = createRuntime({ backgroundReply, services });
    ...
    expect(backgroundReply.startTurn).toHaveBeenCalledTimes(2);
    expect(turns.get('topic-1')?.finish).toHaveBeenCalledWith('completed');
    expect(turns.get('topic-2')?.finish).toHaveBeenCalledWith('completed');
  });

  test('disposes the background reply lifecycle with the session', () => {
    const services = createServices();
    const backgroundReply = createBackgroundReplyLifecycle();
    const runtime = createRuntime({ backgroundReply, services });
