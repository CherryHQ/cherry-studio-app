...
type Pending =
  ...
  | {
      ...
      id: string;
      previousId?: string;
      credential: SlackUserCredential;
      requiresDisconnect: boolean;
    }
  | { status: 'expired' | 'denied'; id: string };

const asCredential = (value: SlackUserCredential): PluginCredential =>
  JSON.parse(JSON.stringify(value));
const cancelled = () => new PluginError('cancelled', 'Slack authorization cancelled.');

/** Owns one in-memory authorization attempt and grant-scoped shared renewal. */
export class SlackAuthorizationRuntime implements PluginAuthorizationRuntime {
  ...
  private async requiresDisconnect(previousId: string | undefined, accountId: string) {
    ...
    try {
      ...
      const parsed = SlackUserCredentialSchema.safeParse(current?.credential);
    ...
    return this.serialize(async () => {
      ...
      if (url.searchParams.has('error')) {
        ...
        if (url.searchParams.get('error') !== 'access_denied') {
          this.pending = undefined;
          throw new PluginError('request', 'Slack could not complete authorization.');
        }
        return this.project();
      }
      ...
      this.pending = undefined;
      const tokens = await slackOauth.exchangeCode(
        pending.application,
        code,
        pending.verifier,
        signal,
      );
      const account = await slackOauth.getAccount(tokens.accessToken, signal);
  ...
  private requireReview(id: string) {
    if (
      ...
      return this.pending;
    throw new PluginError('authorization', 'Slack authorization is no longer available.');
  }
  ...
  private async refreshed(
    ...
    if (credential.rejected)
      throw new PluginError('authorization', 'Slack rejected this authorization.');
    if (
    ...
    if (
      ...
      throw new PluginError('authorization', 'Slack authorization expired.');
    const tokens = await slackOauth.refresh(credential.application, credential.tokens, signal);
    signal.throwIfAborted();
    ...
    return this.serialize(async () => {
      ...
      if (
        ...
        throw new PluginError(
          'requires-disconnect',
          'Disconnect before changing the Slack account.',
        );
      try {
        pending.credential = await this.refreshed(pending.credential, signal);
      } catch (error) {
  ...
  resolveCredential(id: string, callerSignal?: AbortSignal): Promise<PluginCredential> {
    ...
    if (!operation) {
      ...
      operation = this.serialize(async () => {
        ...
        if (failure) throw new PluginError(failure, 'Slack authorization requires reconnecting.');
        const grant = await this.store.getGrant(id).catch((error: unknown) => {
          this.store.notifyChanged();
          throw error;
        });
        const parsed = SlackUserCredentialSchema.safeParse(grant?.credential);
        if (!parsed.success) {
          this.store.notifyChanged();
          throw new PluginError('authorization', 'Slack credentials are missing.');
        }
        ...
        try {
          ...
          if (reason === 'authorization')
            await this.store
              ...
              .catch(() => {});
          this.store.notifyChanged();
          throw new PluginError(reason, 'Slack authorization requires reconnecting.');
        }
      });
  ...
  async describeConnection(id: string): Promise<PluginConnectionStatus> {
    // Local projection only: reading the plugin list never renews a token or checks Slack.
  ...
  async rejectCredential(id: string, rejected: PluginCredential) {
    ...
    await this.serialize(async () => {
      signal.throwIfAborted();
      const current = SlackUserCredentialSchema.safeParse(
        (await this.store.getGrant(id))?.credential,
      );
      const sent = SlackUserCredentialSchema.safeParse(rejected);
      if (
        !current.success ||
        !sent.success ||
