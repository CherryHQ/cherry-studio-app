...
export const feishuTaskTools = [
  defineFeishuApiTool({
    ...
    description:
      'List one page of tasks assigned to the authorized Feishu user (my_tasks), optionally filtered by completion. This is not all tasks created by or visible to the user. Continue with page_token while has_more is true.',
  ...
  defineFeishuApiTool({
    ...
    scopes: ['task:task:read'],
    description:
      'Read a Feishu task by its GUID, including members, deadlines and completion state. Inspect it before updating or completing it.',
  ...
  defineFeishuApiTool({
    ...
    description:
      'Create a Feishu task with explicit members. Use get-user or search-user for member open IDs, including yourself. Start and due timestamps use milliseconds and must use the same all-day setting. Creation may notify members.',
    input: z
      .strictObject({
        ...taskFields,
  ...
  defineFeishuApiTool({
    ...
    scopes: ['task:task:writeonly'],
    description:
      'Update only supplied task fields. Null clears start or due; omitted fields stay unchanged. completed_at is a millisecond timestamp to complete the entire task (all assignees), or "0" to reopen it. Read task_get first; personal completion is not supported.',
  ...
  defineFeishuApiTool({
    ...
    description:
      'Add assignees or followers to an existing Feishu task using verified open IDs. Existing members are preserved; this may notify the added members.',
