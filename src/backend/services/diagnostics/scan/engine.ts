...
function matches(rule: ScanRule, record: LogRecord, text: string): boolean {
  ...
  if (rule.modules && record.module !== undefined && !moduleMatches(record.module, rule.modules))
    return false;
...
function toEvidence(record: LogRecord): FindingEvidence {
  ...
  const excerpt = [
    record.message.slice(0, EXCERPT_MESSAGE_CHARS),
    stackFirstLine?.slice(0, EXCERPT_STACK_CHARS),
  ]
    .filter(Boolean)
    .join(' | ');
  return {
...
export function diagnose(records: Iterable<LogRecord>, options?: DiagnoseOptions): Finding[] {
  ...
  for (const record of records) {
    ...
    for (const rule of rules) {
      ...
      if (accumulator.evidence.length < DEFAULT_MAX_EVIDENCE)
        accumulator.evidence.push(toEvidence(record));
    }
  }
