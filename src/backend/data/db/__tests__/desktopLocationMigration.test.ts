...
it('migrates paired desktops inside a transaction without cascading their cached sessions or retaining auto addresses', () => {
  ...
  try {
    ...
    db.exec(
      'CREATE TABLE connection_test_child(connection_id TEXT REFERENCES desktop_connection(id) ON DELETE CASCADE, scope_id TEXT, session_id TEXT, stream_epoch TEXT, seq TEXT, projection TEXT, updated_at INTEGER)',
    );
    db.exec(`INSERT INTO desktop_connection(id,name,device_id,desktop_identity,addresses,port,grants,status,created_at,updated_at)
      ...
      INSERT INTO connection_test_child(connection_id,scope_id,session_id,stream_epoch,seq,projection,updated_at)
    ...
    expect(db.prepare('SELECT * FROM connection_test_child').get()).toEqual(
      expect.objectContaining({
        scope_id: 'scope',
        seq: '42',
