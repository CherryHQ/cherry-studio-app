...
export const aiUsageRecordTable = sqliteTable(
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.recordKind} IN (${sql.raw(recordKindCheckValues)})`,
    ),
    check(
      'ai_usage_record_message_kind_check',
      sql`${t.messageKind} IN (${sql.raw(messageKindCheckValues)})`,
    ),
    check(
      'ai_usage_record_source_type_check',
      sql`${t.sourceType} IN (${sql.raw(sourceTypeCheckValues)})`,
    ),
    check(
      'ai_usage_record_modality_check',
      sql`${t.modality} IN (${sql.raw(modalityCheckValues)})`,
    ),
    check(
      'ai_usage_record_attribution_check',
      sql`${t.apiKeyAttribution} IN (${sql.raw(attributionCheckValues)})`,
    ),
    check(
      'ai_usage_record_auth_method_check',
      sql`${t.authMethod} IN (${sql.raw(authMethodCheckValues)})`,
    ),
    check(
      'ai_usage_record_cost_source_check',
      sql`${t.costSource} IN (${sql.raw(costSourceCheckValues)})`,
    ),
    check(
      'ai_usage_record_cost_currency_check',
      sql`${t.costCurrency} IN (${sql.raw(costCurrencyCheckValues)})`,
    ),
    ...
    check(
      ...
      sql`${t.cost} IS NULL OR ${t.cost} <= 1.7976931348623157e308`,
    ),
  ],
);
