...
configuration, structured transcript persistence, cancellation, and protocol projection.
`tools: []` is ordinary conversation, and configured tools are adapted from the application-owned
contract into Pi tools. AI SDK may continue to serve non-conversation model capabilities behind those
