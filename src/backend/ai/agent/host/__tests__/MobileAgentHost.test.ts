...
describe('MobileAgentHost', () => {
  ...
  test('retries oversized current and historical images without duplicating transcript messages', async () => {
    const requests: RuntimeExecutionRequest[] = [];
    const fake = new FakeRuntime({ descriptor: FAKE_DESCRIPTOR });
    for (let attempt = 0; attempt < 3; attempt += 1) {
      fake.script((controller) => {
        requests.push(controller.request);
        controller.emit(
          attempt === 1
            ? {
    ...
    const facts = new Map(
      [FILE_ENTRY_ID, SECOND_FILE_ENTRY_ID].map((fileEntryId) => [
        fileEntryId,
        {
          fileEntryId,
          mediaType: 'image/png',
    ...
    const original = 'data:image/png;base64,AAAABBBBCCCCDDDD';
    const compressed = 'data:image/png;base64,AAAA';
    const files: ManagedFileResolver = {
      resolveAvailable: async (ids) => new Map([...facts].filter(([id]) => ids.includes(id))),
      readAsBytes: async () => undefined,
      readDocumentText: async () => undefined,
      readAsDataUrl: async (_file, _signal, options) => (options?.compress ? compressed : original),
    };
    const host = createHost(fake, noOpNaming, files);
    const session = await createStoredSession();
    const events: AgentEvent[] = [];
    await host.observeSession(session.id, (event) => events.push(event));
    for (const fileEntryId of [FILE_ENTRY_ID, SECOND_FILE_ENTRY_ID]) {
      events.length = 0;
      await host.submitMessage({
        ...messageIds(),
        sessionId: session.id,
        parts: [{ type: 'file', fileEntryId, mediaType: 'image/png' }],
      });
      await waitFor(() => terminalTurnEvent(events) !== undefined, 'the image request to settle');
    }
    expect(requests).toHaveLength(3);
    expect(requests[1]?.input[0]).toMatchObject({ uri: original });
    expect(requests[2]?.input[0]).toMatchObject({ uri: compressed });
    expect(requests[1]?.history[0]?.messages[0]?.parts[0]).toMatchObject({ uri: original });
    expect(requests[2]?.history[0]?.messages[0]?.parts[0]).toMatchObject({ uri: compressed });
    ...
    expect(transcript.at(-1)?.status).toBe('success');
    expect(JSON.stringify(transcript)).not.toContain('data:image');
    expect(facts.get(FILE_ENTRY_ID)?.size).toBe(9 * 1024 * 1024);
  });
