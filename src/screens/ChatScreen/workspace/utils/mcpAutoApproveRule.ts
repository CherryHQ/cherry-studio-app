 ...
 *
 * Deliberately writes through `services.mobileMcpServer.update` rather than the
...
export async function clearMcpToolAutoApproveRule(
  services: Pick<DataServices, 'mcp' | 'mobileMcpServer'>,
  source: McpToolSource,
): Promise<void> {
  const server = await services.mobileMcpServer.getById(source.serverId);
  ...
  await services.mobileMcpServer.update(server.id, {
