...
- Pressing Add blurs the text area, dismisses the keyboard, clears the focused state, and opens the
  bottom action sheet.
- The action sheet supports pan-down close and renders: camera, inline iOS PhotosPicker, a divider,
  and the tool action list.
- The inline PhotosPicker exports selected images into temporary files and adds them to the input
  attachment strip immediately.
- Selecting a tool toggles that tool, closes the sheet, and refocuses the
...
- `useChatInputPhotoPicker` owns camera launch and conversion of inline PhotosPicker assets into
  input attachments.
...
4. Press Add and verify the bottom sheet shows Camera, the inline photo picker, and the tool actions.
...
8. Select an image from the inline picker and verify it appears in the input attachment strip.
