...
const ProviderCostWithCurrencySchema = z.union([
  z.object({ cost: finiteNonnegativeCost, currency: z.enum(objectValues(CURRENCY)) }),
  z.object({
    usage: z.object({ cost: finiteNonnegativeCost, currency: z.enum(objectValues(CURRENCY)) }),
  }),
]);
const ProviderCostSchema = z.union([
  z.object({ cost: finiteNonnegativeCost }),
  z.object({ usage: z.object({ cost: finiteNonnegativeCost }) }),
]);
