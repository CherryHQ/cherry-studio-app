...
describe('LanTransferService binary protocol', () => {
  test('handles a complete binary frame', () => {
    ...
    expect(hasher.update).toHaveBeenCalled()
  ...
  test('skips unknown binary frame type', () => {
    ...
    expect(service.addLog).toHaveBeenCalledWith(expect.stringContaining('Unknown binary frame type'), 'error')
  })
