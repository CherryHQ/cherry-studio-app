...
export class BackgroundReplyService implements BackgroundReplyLifecycle {
  ...
  startTurn = (input: BackgroundReplyTurnInput): BackgroundReplyTurn => {
    ...
    const content = deriveBackgroundReplyContent(undefined, i18n.t);
    ...
    return {
      ...
      awaitApproval: (message) => {
        ...
        if (!current) return;
        const latest = deriveBackgroundReplyContent(message, i18n.t);
        current.content = {
          detail: i18n.t('chat.backgroundReply.awaitingApproval'),
  ...
  private updateTurn(
    ...
    const nextContent = deriveBackgroundReplyContent(message, i18n.t);
  ...
  private finishTurn(topicId: string, generation: number, outcome: BackgroundReplyOutcome): void {
    ...
    record.content = getTerminalBackgroundReplyContent(outcome, record.content.preview, i18n.t);
    this.clearUpdateTimer(record);
    void this.enqueue(async () => {
      await this.reconcileAudio();
      if (record.activity) {
  ...
  private toActivityProps(
    ...
    return {
      ...
      compactLabel: getBackgroundReplyCompactLabel(record.content.phase, i18n.t),
  ...
  private async prepareLogo(): Promise<void> {
    try {
      ...
      if (!destination.exists) {
        const asset = await Asset.fromModule(require('../../../assets/icon.png')).downloadAsync();
