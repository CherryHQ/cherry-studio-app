...
import { asciiBytes, createZipBytes } from './_zipTestBytes';

describe('assertZipLimits', () => {
  it('accepts a bounded ZIP archive', () => {
    const bytes = createZipBytes([
      { name: 'word/document.xml', content: asciiBytes('<w:document />') },
    ]);
  ...
  it('prefixes errors with the caller-provided format label', () => {
    ...
    expect(() => assertZipLimits(bytes, 'XLSX')).toThrow(
      /^XLSX preview supports ZIP archives up to/,
    );
  });
});
