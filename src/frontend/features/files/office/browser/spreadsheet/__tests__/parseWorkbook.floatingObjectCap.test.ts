...
import { buildChartWorkbookZip } from './_xlsxTestPackages';
...
describe('parseCharts — floating-object cap and chart part memoization', () => {
  it('caps repeated anchors at the budget and reads the shared chart part only once', async () => {
    ...
    zip.file(
      ...
      repeatedAnchorDrawingXml(MAX_FLOATING_OBJECTS + 36, 'rId1'),
    );

    const fileSpy = jest.spyOn(zip, 'file');
  ...
  it('stops walking anchors once the budget is filled instead of extracting every repeated anchor', async () => {
    ...
    zip.file(
      'xl/drawings/drawing1.xml',
      repeatedAnchorDrawingXml(MAX_FLOATING_OBJECTS * 4, 'rId1'),
    );
    ...
    const colX = jest.fn((col: number) => (col - 1) * DEFAULT_COL_WIDTH_PX);
    const rowY = jest.fn((row: number) => (row - 1) * DEFAULT_ROW_HEIGHT_PX);
  ...
  it('respects an explicit remaining budget, including zero', async () => {
    ...
    const { charts: none, warnings } = await parseCharts(
      zip,
      'Data',
      DEFAULT_LAYOUT,
      emptyDataAccessor,
