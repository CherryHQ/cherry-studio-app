...
describe('createFormulaEvaluator — basic operators and functions', () => {
  it('evaluates four arithmetic operators', () => {
    ...
    expect(evaluator.evaluate('4*3', { ...POS, row: 3 })).toEqual({
      state: 'evaluated',
      value: 12,
    });
    expect(evaluator.evaluate('10/4', { ...POS, row: 4 })).toEqual({
      state: 'evaluated',
      value: 2.5,
    });
  });
  ...
  it('evaluates SUM/AVERAGE/IF/COUNT/ROUND/CONCATENATE over cell references', () => {
    ...
    expect(
      evaluator.evaluate('IF(A1>0,"pos","neg")', { sheet: 'Sheet1', row: 10, col: 3 }),
    ).toEqual({
      state: 'evaluated',
      value: 'pos',
    });
    ...
    expect(
      evaluator.evaluate('CONCATENATE("a","b")', { sheet: 'Sheet1', row: 10, col: 6 }),
    ).toEqual({
      state: 'evaluated',
      value: 'ab',
    });
  ...
  it('evaluates VLOOKUP against a stub table', () => {
    ...
    const { evaluator } = setup(sheets);
    expect(
      evaluator.evaluate('VLOOKUP(2,A1:B3,2,FALSE)', { sheet: 'Sheet1', row: 10, col: 1 }),
    ).toEqual({
      state: 'evaluated',
      value: 'two',
    });
...
describe('createFormulaEvaluator — custom aggregate functions (library stubs)', () => {
  ...
  it('evaluates MAX-minus-MIN composite formula', () => {
    const { evaluator } = setup(statSheet);
    expect(evaluator.evaluate('MAX(C2:C8)-MIN(C2:C8)', at(16))).toEqual({
      state: 'evaluated',
      value: 5,
    });
  });
  ...
  it('MEDIAN averages the two middle values for an even count', () => {
    ...
    expect(evaluator.evaluate('MEDIAN(C3:C8)', at(17))).toEqual({
      state: 'evaluated',
      value: 35.5,
    });
  });
  ...
  it('LARGE with out-of-range k returns #NUM! as an evaluated error', () => {
    const { evaluator } = setup(statSheet);
    expect(evaluator.evaluate('LARGE(C3:C8,99)', at(22))).toEqual({
      state: 'evaluated',
      value: '#NUM!',
    });
  });
  ...
  it('propagates an error cell through custom numeric aggregates', () => {
    ...
    expect(evaluator.evaluate('MAX(A1:A2)', at(24))).toEqual({
      state: 'evaluated',
      value: '#DIV/0!',
    });
    expect(evaluator.evaluate('MEDIAN(A1:A2)', at(25))).toEqual({
      state: 'evaluated',
      value: '#DIV/0!',
    });
  });
  ...
  it('MODE.SNGL returns #N/A when every value is unique', () => {
    ...
    expect(evaluator.evaluate('MODE.SNGL(C3:C8)', at(26))).toEqual({
      state: 'evaluated',
      value: '#N/A',
    });
  });
...
describe('createFormulaEvaluator — oversized range guard', () => {
  it('returns unevaluated for a whole-sheet range without materializing any cell', () => {
    ...
    expect(
      evaluator.evaluate('SUM(A1:XFD1048576)', { sheet: 'Sheet1', row: 1, col: 20000 }),
    ).toEqual({
      state: 'unevaluated',
    });
