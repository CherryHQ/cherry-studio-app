...
import { createZipBytes } from '../../__tests__/_zipTestBytes';
...
import { buildChartWorkbookArrayBuffer } from './_xlsxTestPackages';
...
describe('parseWorkbook — per-sheet defaults + cell-less row definitions', () => {
  ...
  beforeAll(async () => {
    ...
    zip.file(
      ...
      sheetXml.replace(
        '</sheetData>',
        '<row r="7" hidden="1"/><row r="8" ht="30" customHeight="1"/></sheetData>',
      ),
    );
...
describe('parseWorkbook — formulas: shared formula text is translated per cell', () => {
  ...
  beforeAll(async () => {
    ...
    ws.getCell('O7').value = {
      sharedFormula: 'O6',
      result: 5,
    } satisfies ExcelJS.CellSharedFormulaValue;
...
describe('parseWorkbook — ZIP preflight rejects decompression bombs', () => {
  it('rejects an entry whose declared uncompressed size exceeds the limit', async () => {
    ...
    await expect(parseWorkbook(bytes.buffer, 'bomb-entry.xlsx')).rejects.toThrow(
      /XLSX preview supports ZIP entries/,
    );
  });
  ...
  it('rejects an archive whose declared total uncompressed size exceeds the limit', async () => {
    ...
    await expect(parseWorkbook(bytes.buffer, 'bomb-total.xlsx')).rejects.toThrow(
  ...
  it('rejects an archive declaring too many entries', async () => {
    ...
    await expect(parseWorkbook(bytes.buffer, 'bomb-entries.xlsx')).rejects.toThrow(
      /up to 4000 entries/,
    );
  });
});
