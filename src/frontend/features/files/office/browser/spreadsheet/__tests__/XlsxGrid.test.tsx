...
import XlsxGrid, { type SelectedCellInfo } from '../XlsxGrid';
import { createMockWorkbookModel } from './_mockWorkbookModel';

// jsdom has no layout. Stub only the virtual viewport and assert DOM selection, merges,
// accessible coordinates, image placements and chart cleanup. Device acceptance owns pixel fidelity.
...
const mockGrid = {
  ...
  rowScrollToIndex: jest.fn(),
  colScrollToIndex: jest.fn(),
  useVirtualizer: jest.fn(),
};

jest.mock('@tanstack/react-virtual', () => ({
  useVirtualizer: (...args: unknown[]) => mockGrid.useVirtualizer(...args),
}));
...
const setRangeFromCounts = (
  ...
  mockGrid.rowRange = toVirtualItems('row', rowIndexes, rowSize, rowStart);
  mockGrid.colRange = toVirtualItems('col', colIndexes, colSize, colStart);
};

const virtualizerImpl = (options: any) => {
  const isHorizontal = Boolean(options.horizontal);
  if (isHorizontal) {
    mockGrid.lastColOptions = options;
  } else {
    mockGrid.lastRowOptions = options;
  }
  return {
    measure: jest.fn(),
    getVirtualItems: () => (isHorizontal ? mockGrid.colRange : mockGrid.rowRange),
    getTotalSize: () => {
      const range = isHorizontal ? mockGrid.colRange : mockGrid.rowRange;
      const last = range[range.length - 1];
      return last ? last.start + last.size : 0;
    },
    scrollToIndex: isHorizontal ? mockGrid.colScrollToIndex : mockGrid.rowScrollToIndex,
  };
};
...
beforeEach(() => {
  jest.clearAllMocks();
  mockGrid.useVirtualizer.mockImplementation(virtualizerImpl);
});
...
describe('XlsxGrid — mock model rendering', () => {
  ...
  it('passes precise estimateSize functions backed by the sheet layout (no measureElement reliance)', () => {
    ...
    expect(mockGrid.lastRowOptions.estimateSize(0)).toBe(36);
    expect(mockGrid.lastRowOptions.estimateSize(1)).toBe(20);
    // Row 7 (index 6) is hidden (height 0 override).
    expect(mockGrid.lastRowOptions.estimateSize(6)).toBe(0);
    // Col A (index 0) has an explicit override (110px); col E (index 4) is hidden (0).
    expect(mockGrid.lastColOptions.estimateSize(0)).toBe(110);
    expect(mockGrid.lastColOptions.estimateSize(4)).toBe(0);
  });
  ...
  it('extends the grid with blank rows/cols beyond the used range at default sizes', () => {
    ...
    expect(mockGrid.lastRowOptions.count).toBeGreaterThan(salesSheet.rowCount);
    expect(mockGrid.lastColOptions.count).toBeGreaterThan(salesSheet.colCount);
    // Padded rows/cols beyond the used range fall back to the sheet defaults.
    expect(mockGrid.lastRowOptions.estimateSize(salesSheet.rowCount)).toBe(20);
    expect(mockGrid.lastColOptions.estimateSize(salesSheet.colCount)).toBe(64);
  });
  ...
  it('grows the padded range to fill a viewport larger than the used range', () => {
    ...
    expect(mockGrid.lastRowOptions.count).toBeGreaterThanOrEqual(100);
    expect(mockGrid.lastColOptions.count).toBeGreaterThanOrEqual(32);
  });
});
...
describe('XlsxGrid — zoom', () => {
  ...
  it('feeds the virtualizer scroll-space (scaled) sizes while rendering unscaled coordinates', () => {
    ...
    expect(mockGrid.lastRowOptions.estimateSize(0)).toBe(72);
    expect(mockGrid.lastColOptions.estimateSize(0)).toBe(220);
  });
});
...
describe('XlsxGrid — keyboard selection', () => {
  ...
  it('scrolls the moved-to cell into view (virtualized cells may be unmounted)', () => {
    ...
    expect(mockGrid.rowScrollToIndex).toHaveBeenLastCalledWith(1);
    expect(mockGrid.colScrollToIndex).toHaveBeenLastCalledWith(0);
  });
...
describe('XlsxGrid — grid semantics', () => {
  ...
  it('keeps visual padding outside the semantic grid', () => {
    ...
    expect(screen.getAllByRole('gridcell')).toHaveLength(
      model.sheets[1].rowCount * model.sheets[1].colCount,
    );
  });
...
describe('XlsxGrid — floating layer', () => {
  ...
  it('invokes renderChart for a supported chart and calls the returned dispose function on unmount', () => {
    ...
    const renderChart = jest.fn((chart: ChartModel, container: HTMLElement) => {
      void chart;
      void container;
      return dispose;
    });
  ...
  it('renders the unsupported chart placeholder with the raw type name instead of calling renderChart', () => {
    ...
    const renderChart = jest.fn(() => jest.fn());
    render(
      <XlsxGrid
        labels={GRID_LABELS}
        sheet={salesSheet}
    ...
    expect(
      screen.getByText('scatterChart').closest('[data-testid="xlsx-grid-chart"]')
        ?.firstElementChild,
    ).toHaveClass('office-chart-unsupported');
  ...
  it('falls back to the unsupported placeholder for a supported chart type when no renderChart is provided', () => {
    ...
    expect(screen.getByTestId('xlsx-grid-chart').firstElementChild).toHaveClass(
      'office-chart-unsupported',
    );
  });
});
