...
const cacheXml = <T extends string | number>(
  tagName: 'strCache' | 'numCache',
  values: T[],
): string => `
  <${tagName}>
...
${Array.from(
  { length: chartCount },
  (_, i) =>
    `  <Relationship Id="rId${i + 1}" Type="${DRAWING_REL_NS}/chart" Target="../charts/chart${i + 1}.xml"/>`,
).join('\n')}
</Relationships>`;
...
export const buildChartWorkbookZip = async (kind: ChartWorkbookKind): Promise<JSZip> => {
  ...
  if (contentTypes) {
    let updated = addContentTypeOverride(
      contentTypes,
...
export const buildChartWorkbookArrayBuffer = async (
  kind: ChartWorkbookKind,
): Promise<ArrayBuffer> => {
