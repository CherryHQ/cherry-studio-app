...
export interface XlsxGridProps {
  labels: { chart: string; unsupportedChart: string; imageUnavailable: string };
...
const borderEdgeToCss = (edge: BorderEdge | undefined): string | undefined =>
  edge
    ? `${BORDER_WIDTH_PX[edge.style]}px ${BORDER_STYLE_CSS[edge.style]} ${edge.color}`
    : undefined;

const H_ALIGN_TO_JUSTIFY: Record<
  NonNullable<CellStyle['hAlign']>,
...
const V_ALIGN_TO_ITEMS: Record<
  NonNullable<CellStyle['vAlign']>,
...
const CellView = memo(function CellView({
  ...
  const finalCss: React.CSSProperties = {
    ...
    borderTop: covered
      ? undefined
      : isFirstRow
        ? (css.borderTop ?? DEFAULT_GRID_LINE)
        : css.borderTop,
    borderLeft: covered
      ? undefined
      : isFirstCol
        ? (css.borderLeft ?? DEFAULT_GRID_LINE)
        : css.borderLeft,
  };

  return (
    <div className="office-cell" style={finalCss}>
      {cell && (
        <span
          className={cell.formulaState === 'unevaluated' ? 'office-unevaluated' : undefined}
...
const SelectedCellOverlay = ({ cell, style, rect }: SelectedCellOverlayProps) => {
  ...
  return (
    <div data-testid="xlsx-grid-selected-overlay" className="office-cell-overlay" style={finalCss}>
      {cell && (
        <span
          className={cell.formulaState === 'unevaluated' ? 'office-unevaluated' : undefined}
...
const UnsupportedChartPlaceholder = ({ chart, label }: UnsupportedChartPlaceholderProps) => {
  return (
    <div className="office-chart-unsupported">
      <span>{label}</span>
...
const ChartHost = ({ chart, renderChart, label }: ChartHostProps) => {
  ...
  return <div ref={setRef} className="office-fill" role="img" aria-label={chart.title || label} />;
};
...
const XlsxGrid = ({
  ...
  'use no memo'; // TanStack Virtual requires live imperative reads; keep its desktop memo boundary.
  ...
  const renderedRowCount =
    Math.max(sheet.rowCount, Math.ceil(viewportHeight / (sheet.defaultRowHeightPx * zoom))) +
    EXTRA_ROWS;
  const renderedColCount =
    Math.max(sheet.colCount, Math.ceil(viewportWidth / (sheet.defaultColWidthPx * zoom))) +
    EXTRA_COLS;
  ...
  const zoomTransform: React.CSSProperties = {
    transform: `scale(${zoom})`,
    transformOrigin: 'top left',
  };

  // eslint-disable-next-line react-hooks/incompatible-library -- This owner opts out with use no memo; live virtualizer reads are deliberate.
  ...
  // Cached estimates must follow zoom and resized row/column layouts.
  useEffect(() => {
    rowVirtualizer.measure();
    colVirtualizer.measure();
  }, [rowVirtualizer, colVirtualizer, rowLayout, colLayout, zoom]);
  ...
  const getCell = useCallback(
    (row: number, col: number) => sheet.cells[`${row}:${col}`],
    [sheet.cells],
  );
  const getStyle = useCallback(
    (cell: CellRenderModel | undefined) =>
      cell?.styleId !== undefined ? styles[cell.styleId] : undefined,
    [styles],
  );
  ...
  const findMerge = useCallback(
    (row: number, col: number) => {
      ...
      return sheet.merges.find(
        (merge) =>
          row >= merge.top && row <= merge.bottom && col >= merge.left && col <= merge.right,
      );
    },
  ...
  const selectCell = useCallback(
    (row: number, col: number) => {
      ...
      onSelectCell?.({
        address: cellAddress(masterRow, masterCol),
        cell: getCell(masterRow, masterCol) ?? null,
      });
    },
  ...
  return (
    <div
      ...
      tabIndex={0}
    >
      <div className="office-position-relative" style={{ width: totalWidth, height: totalHeight }}>
        {/* Column header (sticky top): A, B, C... The box is positioned in scroll coordinates; content is scaled. */}
        <div
          className="office-column-headers"
          style={{
        ...
        >
          <div className="office-position-absolute" style={zoomTransform}>
            {virtualCols.map((vc) => (
              <div
                key={vc.key}
                className="office-column-header"
                style={{
        ...
        >
          ...
          <div className="office-position-absolute">
            {virtualRows.map((vr) => {
                  ...
                  {virtualCols.map((vc) => {
                    ...
                    return (
                      <div
                        ...
                        aria-colspan={
                          isMaster && exposesGridCell ? merge.right - merge.left + 1 : undefined
                        }
                        aria-rowspan={
                          isMaster && exposesGridCell ? merge.bottom - merge.top + 1 : undefined
                        }
                        aria-label={
                          isMaster && exposesGridCell
                            ? getCell(row, col)?.text || undefined
                            : undefined
                        }
                      >
                        <CellView
          ...
          <div className="office-floating-layer">
            {sheet.floatingImages.map((img, i) => {
              ...
              return (
                <FloatingImage
                  ...
                  // oxlint-disable-next-line react/no-array-index-key -- Immutable anchor order identifies repeated uses of one image.
                  key={`img:${img.imageId}:${i}`}
                  src={src}
                  label={labels.imageUnavailable}
                  onWarning={onWarning}
                  data-testid="xlsx-grid-floating-image"
                  className="office-floating-image"
                  style={{
                    top: img.rect.y,
                    left: img.rect.x,
            ...
            {sheet.charts.map((chart, i) => {
              ...
              return (
                <div
                  // oxlint-disable-next-line react/no-array-index-key -- Charts have no IDs and their order is immutable within this sheet.
                  key={`chart:${i}`}
                  className="office-floating-object"
                  style={positionStyle}
                  data-testid="xlsx-grid-chart"
                >
                  {chart.type === 'unsupported' || !renderChart ? (
                    <UnsupportedChartPlaceholder chart={chart} label={labels.unsupportedChart} />
                  ) : (
                    <ChartHost chart={chart} renderChart={renderChart} label={labels.chart} />
                  )}
                </div>
              );
            })}
...
function FloatingImage({
  ...
  return failed ? (
    <div {...props} role="img" aria-label={label}>
      <div className="office-image-unavailable">{label}</div>
    </div>
  ) : (
    <img
      {...props}
