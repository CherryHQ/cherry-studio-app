 ...
 */
export const createChartSheetPartPathMap = async (
  zip: JSZip,
...
const resolveDrawingPartPath = async (
  zip: JSZip,
...
const readCache = (
  ...
  if (!Number.isInteger(ptCount) || ptCount < 0 || ptCount > MAX_CHART_CACHE_POINTS) {
    warnings.push(
      `chart cache declares an out-of-range ptCount (${ptCount}); falling back to cell references`,
    );
    return null;
  }
  ...
  for (const pt of pts) {
    ...
    if (!Number.isInteger(idx) || idx < 0 || idx >= MAX_CHART_CACHE_POINTS) {
      warnings.push(
        `chart cache point index out of range (${idx}); falling back to cell references`,
      );
      return null;
    }
...
const parseSeries = (
  ...
  const categories: (string | number)[] =
    catValues.length > 0
      ? catValues.map((c) => (c === null ? '' : c))
      : normalizedValues.map((_, i) => i + 1);
...
const pickChartTypeNode = (
  plotArea: unknown,
  warnings: string[],
): { tag: string; node: unknown } | null => {
...
export async function parseCharts(
  ...
  try {
    ...
    const { charts: anchoredCharts, skippedAnchors } = extractAnchoredCharts(
      drawingXml,
      layout,
      chartBudget,
    );
