...
function isRichTextValue(value: unknown): value is RichTextCellValue {
  return (
    typeof value === 'object' &&
    value !== null &&
    Array.isArray((value as RichTextCellValue).richText)
  );
}
 ...
 */
async function normalizeDrawingsForExcelJs(
  zip: JSZip,
...
export async function parseWorkbook(
  ...
  const chartSheetPartPathsPromise = hasDrawingParts
    ? createChartSheetPartPathMap(zip).catch((err) => {
        warnings.add(
          `chart-workbook-index-failed:${err instanceof Error ? err.message : String(err)}`,
        );
  ...
  const readParsedCellValue: CellValueReader = (sheetName, row, col) => {
    const dateSerial = formulaDateSerialsBySheet
      .get(sheetName)
      ?.get(row * FORMULA_DATE_KEY_STRIDE + col);
      ...
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        ...
        if (isFormulaValue(value)) {
          ...
          if (hasResult) {
            ...
            if (dateResult) {
              setFormulaDateSerial(
                worksheet.name,
                rowNumber,
                colNumber,
                dateToExcelSerial(dateResult),
              );
            }
          } else if (formulaText) {
  ...
  if (pendingFormulas.length > 0 && date1904) {
    ...
    // evalContext and evaluator reference each other, so declare before assignment.
