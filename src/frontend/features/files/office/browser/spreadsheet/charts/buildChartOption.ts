...
const buildCategoryAxisData = (series: ChartSeries[]): (string | number)[] =>
  series[0]?.categories ?? [];
...
const normalizePercentStacked = (series: ChartSeries[]): (number | null)[][] => {
  ...
  const totals = Array.from({ length: categoryCount }, (_, i) =>
    series.reduce((sum, s) => sum + (s.values[i] ?? 0), 0),
  );
  return series.map((s) =>
    s.values.map((v, i) => (v === null || totals[i] === 0 ? null : (v / totals[i]) * 100)),
  );
};
...
const buildBarLineAreaOption = (chart: ChartModel): EChartsCoreOption => {
  ...
  const seriesValues = isPercentStacked
    ? normalizePercentStacked(chart.series)
    : chart.series.map((s) => s.values);
