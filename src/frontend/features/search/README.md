...
This page owns presentation and route lifecycle for the transient `/search` route.

`SearchScreen.tsx` renders the request supplied by `frontend/appShell/search`. The App Shell module
owns the cross-page opening action and keeps non-serializable callbacks in memory.
...
it can match keeps its search in place with
[Inline Search](../../components/InlineSearch/README.md) instead.
...
different interaction contract owns a separate page instead of becoming another App Search mode —
persistent in-place search is [Inline Search](../../components/InlineSearch/README.md), and
multi-selection search belongs to whichever page needs it.
