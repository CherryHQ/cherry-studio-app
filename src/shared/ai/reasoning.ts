...
export function nearestThinkingOption(
  ...
  const selectable = options.filter(
    (option): option is ReasoningEffortOption => option !== 'default',
  );
...
export function resolveReasoningEffortForModel(
  ...
  if (currentEffort !== undefined)
    return nearestThinkingOption(currentEffort, supportedOptions) ?? supportedOptions[0];
...
export function nearestEffortForBudget(
  ...
  if (!Number.isFinite(budget) || tokenLimit?.min == null || tokenLimit.max == null)
    return undefined;
