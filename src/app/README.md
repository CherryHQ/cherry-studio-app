...
`src/app` is intentionally thin. Feature implementation belongs under `src/features`; reusable
cross-feature modules belong under `src/components`.

## Ownership

- Keep only route files, route groups, dynamic route folders, `_layout.tsx`, and this `README.md`
  here.
- Put feature composition, private components, hooks, context, utils, and tests in `src/features`.
- Put reusable app-shell or cross-feature modules in `src/components`.
...
owning `src/features/*` module and keep the route as an adapter.
...
- Route files import features from module roots, for example `@/features/settings`.
