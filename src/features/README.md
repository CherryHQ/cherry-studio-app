...
- Route files import from feature module roots (`@/features/<name>`).
...
- Cross-feature imports go through a public surface: `@/features/<name>` or
  `@/features/<name>/<area>` (an area with a deliberate `index.ts`, e.g. `@/features/chat/input`,
  `@/features/chat/workspace`). Deep value imports past that are lint errors; type-only deep
  imports are allowed.
...
- Cross-feature reusable modules with two or more feature owners belong in neutral
  `src/components`, `src/hooks`, or `src/utils` — move them when the second owner appears.
- Do not import feature-private modules from `src/components` (shared layers never depend on
  features).
...
Reusable modules that remain in `src/components` include app shell modules (`headers`,
...
behavior such as `confirmDialog`, and native dependency adapters such as `nativePrimitives`.
