...
function checkClaudeSkillSymlink(skillName: string, errors: string[]) {
  ...
  if (actualTarget !== expectedTarget) {
    errors.push(
      `.claude/skills/${skillName} symlink points to '${actualTarget}', expected '${expectedTarget}'`,
    );
  }
}
...
function checkTrackedFilesAgainstWhitelist(skillNames: string[], errors: string[]) {
  ...
  const allowedClaudeSymlinks = new Set(
    skillNames.map((skillName) => `.claude/skills/${skillName}`),
  );
  ...
  for (const file of trackedFiles) {
    ...
    if (file.startsWith('.claude/skills/')) {
      ...
      if (
        allowedClaudeSymlinks.has(file) ||
        allowedClaudePrefixes.some((prefix) => file.startsWith(prefix))
      ) {
        continue;
      }
      errors.push(`tracked file is outside public skill whitelist: ${file}`);
...
function main() {
  ...
  checkGitignore(
    AGENTS_SKILLS_GITIGNORE,
    buildAgentsSkillsGitignore(skillNames),
    '.agents/skills/.gitignore',
    errors,
  );
  checkGitignore(
    CLAUDE_SKILLS_GITIGNORE,
    buildClaudeSkillsGitignore(skillNames),
